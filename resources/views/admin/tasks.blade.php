...
<div class="max-w-7xl mx-auto">
    ...
    <div x-data="{ 
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <div class="flex flex-col sm:flex-row flex-wrap items-stretch sm:items-center gap-2 w-full sm:w-auto mt-4 sm:mt-0">
                <div x-show="selectionMode" x-transition style="display: none;" class="w-full sm:w-auto">
                    <form action="{{ route('tasks.bulk-destroy') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tugas yang Anda pilih? Tindakan ini tidak dapat dibatalkan.');" class="w-full">
                        @csrf
                        @method('DELETE')
                        <template x-for="id in selectedTasks" :key="id">
                            <input type="hidden" name="task_ids[]" :value="id">
                        </template>
                        <button type="submit" :disabled="selectedTasks.length === 0" class="w-full inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none border border-destructive bg-destructive text-destructive-foreground hover:bg-destructive/90 h-10 px-4 shadow disabled:opacity-50">
                ...
                <button x-show="!selectionMode" @click="openCreate()" class="w-full sm:w-auto inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 shadow">
                        ...
                        <div class="border rounded-lg p-4 bg-background">
                            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                                <div class="flex items-center gap-3 min-w-0 flex-1">
                                    ...
                                    @if($task->is_draft)
                                        <span class="inline-flex items-center rounded-md border px-2.5 py-0.5 text-xs font-bold bg-orange-600 text-white shadow-sm shadow-orange-500/50 uppercase tracking-wider animate-pulse">DRAFT</span>
                                    @endif
                                </div>
                ...
                <form :action="isEdit ? '{{ url('tasks') }}/' + taskId : '{{ route('tasks.store') }}'" method="POST" enctype="multipart/form-data" @submit.prevent="submitForm">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="pt-4 flex justify-end gap-2">
                            ...
                            <button type="submit" @click="isDraft = 1" :disabled="isUploading" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium border border-input bg-background hover:bg-accent h-10 px-4 disabled:opacity-50">
                                Simpan Draft
                            </button>
                            <button type="submit" @click="isDraft = 0" :disabled="isUploading" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-6 disabled:opacity-50 min-w-[140px]">
