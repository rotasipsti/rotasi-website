        ...
        <div class="rounded-xl border border-border/50 bg-card text-card-foreground shadow">
            <div class="flex flex-col space-y-1.5 p-6 text-center">
                ...
                <p class="text-sm text-muted-foreground">
                    Masukkan data untuk login akun panitia ROTASI
                </p>
            </div>
            <div class="p-6 pt-0">
                <form method="POST" action="{{ route('login.u') }}" class="mt-6">
                    @csrf
                    ...
                    <div class="space-y-4">
                        ...
                        <div x-data="{ showPassword: false }">
                            ...
                            <div class="relative mt-2">
                                <input id="password" :type="showPassword ? 'text' : 'password'" name="password" placeholder="Masukkan password" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 pr-10" />
                ...
                <div class="mt-4 text-center text-sm text-muted-foreground">
                    Belum punya akun? <a href="{{ route('register.u') }}" class="text-primary hover:underline">Daftar disini</a>
                </div>
            </div>
        </div>
    </div>
